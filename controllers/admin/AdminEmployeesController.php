...
class AdminEmployeesControllerCore extends AdminController
{
    ...
    public function processStatus()
    {
        ...
        if($employee->id_profile == 5)
        {
            $supplier = new Supplier($employee->id_supplier);
            if($employee->active == 0 && $employee->id_supplier != NULL)
            {
                $supplier->active = 1;
                $supplier->save();
            }
            if($employee->active == 1 && $employee->id_supplier != NULL)
            {
                $supplier->active = 0;
                $supplier->save();
            }
